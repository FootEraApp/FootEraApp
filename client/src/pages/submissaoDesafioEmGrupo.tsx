...
export default function SubmissaoDesafioEmGrupo() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`${API.BASE_URL}/api/desafios/em-grupo/${grupoId}/ativo`, {
          headers: { Authorization: `Bearer ${Storage.token || ""}` },
        });
        if (!r.ok) {
          setErrorMsg("Nenhum desafio em grupo ativo para este grupo.");
          return;
        }
        const data = await r.json();
        setDesafioEmGrupoId(data.id);
      } catch {
        setErrorMsg("Falha ao carregar o desafio em grupo ativo.");
      }
    })();
  }, [grupoId, desafioId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!videoFile) return setErrorMsg("Selecione um vídeo para enviar.");
    if (!desafioEmGrupoId) return setErrorMsg("Desafio em grupo não encontrado.");
    ...
    <div className="max-w-xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input type="file" accept="video/*" onChange={handleFileChange} className="border p-2 rounded w-full" />
        </div>
