...
export default function CreatorProfile() {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API.BASE_URL}/api/creator/profile/${usuarioId}`,
      {
        headers: {
          Authorization:
            `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    fetch(
      `${API.BASE_URL}/api/eventos?creatorUsuarioId=${encodeURIComponent(
        usuarioId
      )}`,
      {
        headers:
  ...
  async function ativarMeuCreator() {
    ...
    try {
      const resposta =
        await fetch(
          `${API.BASE_URL}/api/creator/ativar`,
          {
            method:
              "POST",
