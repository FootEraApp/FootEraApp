...
function ProfilePostsSection({ usuarioId }: { usuarioId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("Erro ao carregar postagens do perfil único:", e);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 mt-6 pb-6 space-y-4">
      <h2 className="text-lg font-semibold text-green-900">Postagens</h2>

      {loadingPosts && (
        <p className="text-sm text-gray-500">Carregando postagens...</p>
      )}

      {!loadingPosts && posts.length === 0 && (
        <p className="text-sm text-gray-500">Nenhuma postagem ainda.</p>
      )}
