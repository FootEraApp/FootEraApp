...
export default function ProfileHeader({
  ...
  const obsKey =
    Storage.usuarioId && alvoAtletaId
      ? `obs_${Storage.usuarioId}_${alvoAtletaId}`
      : null;
  ...
  const isDonoOrganizador = ["professor", "clube", "escola", "escolinha"].includes(
    tipoUsuarioAtual
  );
  const donoId = (Storage as any).tipoUsuarioId as string | undefined;
  ...
  useEffect(() => {
    ...
    fetch(`${API.BASE_URL}/api/perfil/${encodeURIComponent(perfilId)}`, {
      ...
      .then((data) => {
        ...
        const isAtleta =
          id || String(data?.tipo || "").toLowerCase() === "atleta";
    ...
    (async () => {
      try {
        const r1 = await fetch(
          `${API.BASE_URL}/api/seguidores/status?seguidoUsuarioId=${encodeURIComponent(
            perfilId
          )}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  async function cancelarSolicitacaoTreino(usuarioAlvoId: string) {
    ...
    return (
      delBody.ok || delBody.status === 204 || delBody.status === 404
    );
  }
  ...
  // ---------- DESVINCULAR (PROFESSOR/CLUBE/ESCOLINHA) ----------
  async function desvincularAtleta() {
    ...
    if (!donoId) {
      alert("Não foi possível identificar o professor/clube/escolinha logado.");
      return;
    }

    const atletaIdReal = await resolverAtletaIdSePreciso();
    if (!atletaIdReal) {
      alert("Não foi possível identificar o atleta para desvincular.");
      return;
    }
    ...
    if (tipoUsuarioAtual === "clube") {
      baseUrl = `${API.BASE_URL}/api/clubes/${encodeURIComponent(
        donoId
      )}/desvincular-atleta`;
    } else if (
      tipoUsuarioAtual === "escola" ||
      tipoUsuarioAtual === "escolinha"
    ) {
      baseUrl = `${API.BASE_URL}/api/escolinhas/${encodeURIComponent(
        donoId
      )}/desvincular-atleta`;
    }
    ...
    const resp = await fetch(baseUrl, {
      ...
      body: JSON.stringify({ atletaId: atletaIdReal }),
    });

    if (resp.ok) {
      setTreinoJunto(false);
      setTreinoStatus("DESVINCULADO");
      alert("Atleta desvinculado com sucesso. O histórico foi salvo.");
    } else {
      const body = await readBodySafe(resp);
      const msg =
        body?.message ||
        body?.error ||
        "Não foi possível desvincular o atleta agora.";
      alert(msg);
    }
  }

  function abrirConfirmDesvincular() {
    setConfirmBox({
      open: true,
      text:
        "Tem certeza que deseja desvincular este atleta? " +
        "O histórico desse vínculo será salvo e ele deixará de aparecer como atleta vinculado.",
      onYes: async () => {
        await desvincularAtleta();
      },
    });
  }
  ...
  const btnBase =
    "rounded-full font-semibold focus:outline-none focus:ring-2 focus:ring-white/40 transition inline-flex items-center justify-center gap-2";

  const treinoBtnLabel =
    treinoStatus === "ATIVO"
      ? "Já treino junto"
      : treinoStatus === "DESVINCULADO"
      ? "Desvinculado"
      : treinoStatus === "CARREGANDO"
      ? "..."
      : "Treinar juntos";

  const treinoBtnTitle =
    treinoStatus === "ATIVO"
      ? "Clique para desvincular este atleta (e salvar histórico)."
      : treinoStatus === "DESVINCULADO"
      ? "Atleta desvinculado recentemente. Você ainda pode solicitar um novo treino."
      : treinoStatus === "CARREGANDO"
      ? "Carregando status de treino..."
      : "Solicitar treino em conjunto";
      ...
      {!isOwnProfile && (
        <div className="w-full max-w-2xl px-3 mt-4 mb-2">
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            ...
            <button
              disabled={treinoStatus === "CARREGANDO"}
              aria-pressed={treinoStatus === "ATIVO"}
              onClick={toggleTreino}
              className={`${btnBase} ${treinoBtnExtraClass} 
                    disabled:opacity-60 disabled:cursor-not-allowed
                    px-3 py-1.5 text-xs md:px-4 md:py-2 md:text-sm`}
              title={treinoBtnTitle}
            >
              <Users size={16} />
              <span className="truncate">{treinoBtnLabel}</span>
            </button>
