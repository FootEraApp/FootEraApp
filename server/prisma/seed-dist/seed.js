import { PrismaClient, TipoUsuario, TipoTreino, Nivel, Categoria, OrigemFormador, MetodoPagamento, Periodicidade, TipoMensagem, PagamentoStatus, PosicaoCampo, } from '@prisma/client';
...
async function main() {
    ...
    if (usuarioAtletaPro) {
        await garantirAssinaturaFixa(usuarioAtletaPro.id, 'ATLETA_PRO', Periodicidade.Mensal);
    }
    if (usuarioProfPro) {
        await garantirAssinaturaFixa(usuarioProfPro.id, 'PROFESSOR_PRO', Periodicidade.Mensal);
    }
    if (usuarioScoutPro) {
        await garantirAssinaturaFixa(usuarioScoutPro.id, 'OLHEIRO_PRO', Periodicidade.Mensal);
    }
    if (usuarioEscolinha01Db) {
        await garantirAssinaturaFixa(usuarioEscolinha01Db.id, 'ESCOLINHA_PRO', Periodicidade.Mensal);
    }
